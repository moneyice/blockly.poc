...
//MY       ---------------------------------------------------------------------------------------------------------------------------------  
Blockly.Variables.createVariableButtonHandlerMY = function (
  workspace, newVar, opt_callback, opt_type) {
  var type = opt_type || '';
  // This function needs to be named so it can be called recursively.
  var promptAndCheckWithAlert = function (defaultName) {
    Blockly.Variables.MyPromptName(Blockly.Msg['NEW_VARIABLE_TITLE'], defaultName, newVar,
      function (text) {
        if (text) {
          var existing =
            Blockly.Variables.nameUsedWithAnyType(text, workspace);
...
Blockly.Variables.MyPromptName = function (promptText, defaultText, newVar, callback) {
  // Blockly.prompt(promptText, defaultText, function (newVar) {------------------放弃调用prompt弹框去获取参数名-------------
  // Merge runs of whitespace.  Strip leading and trailing whitespace.
  // Beyond this, all names are legal.
  //此处 ----------------------------自行传入参数名-----------------------------
